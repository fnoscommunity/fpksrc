...
if [ -n ${OLD_FPK_REV} ] && [ ${OLD_FPK_REV} -lt 11 ]; then

cat <<EOF > $SYNOPKG_TEMP_LOGFILE
[{
    "step_title": "Update Configuration from ${SYNOPKG_OLD_PKGVER}",
    "items": [{
        "desc": "The configuration file for Redis v7.x will be installed as <code>/var/apps/redis/var/redis.conf.new</code>."
      }, {
        ...
        "desc": "<b>Note:</b> Check <code>/var/apps/redis/var/redis.log</code> to find configuration issues."
      }
    ]
  }
]
