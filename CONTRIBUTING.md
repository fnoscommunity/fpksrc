...
- The Build rule `all-supported` builds the currently supported archs with the default toolchain version. All affected archs must successfully build - or be declared as UNSUPPORTED_ARCHS in the fpk/ Makefile of the package.
...
- A new installation must succeed. There must not appear error messages while installation or in the install log file found in `/var/log/apps/{package-name}.log` (for DSM<=5 in `/var/apps/{package-name}/target/var/{package-name}_install.log`).
